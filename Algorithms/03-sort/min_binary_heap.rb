...
class MinBinaryHeap
  ...
  def printf
    puts collect_nodes().map { |node| "#{node.data}"  }.join "\n"
  end
  
  def returnArray
    collect_nodes().map { |node| node.data }
  end
      ...
      parent_row.each_with_index do |curr_parrent, index|
        if node.parent == curr_parrent
          if curr_parrent.right == node
            node.data > curr_parrent.left.data ? swap(node, curr_parrent, nil, "same_parent") : return
          end
          if curr_parrent.left == node
            lefter_parent = next_parent(parent_row, index)

            if lefter_parent != @root
              node.data > lefter_parent.right.data ? swap(node, curr_parrent, lefter_parent, "diff_parent") : return
              min_sort(node) if lefter_parent == parent_row.first.parent
            elsif lefter_parent == @root
              node.data > @root.data ? swap(node, curr_parrent, @root, "new_root") : return
            end
          end
        end
      end
